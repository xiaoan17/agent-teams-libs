...
#### Local run environment (this install)

> Dependencies are pre-installed in an isolated venv at the agent root
> (`07_design_human/.venv`), and `.env` (API key + custom endpoint) already
> sits in this skill directory. **Always run the Python scripts via the
> `run.sh` wrapper** — it activates the venv and puts the Homebrew `cairo`
> dylib on the loader path (required by `cairosvg`); a bare `python scripts/...`
> will fail with "no library called cairo-2 was found".

```bash
# from this skill directory
./run.sh scripts/svg_to_png.py logo.svg logo.png
./run.sh scripts/generate_showcase.py --all-styles --reference logo.png
```
